...
int main()
{
	...
	while (true)
	{
		...
		line.append(delimiter);*/ //adding delimiter
		// Copy the line into the buffer, filling the rest with dashes.
		// (We must be careful not to write past the end of the array.)
		//memset(buffer, '-', MESSAGESIZE); //no need of it
		memcpy(buffer, line.c_str(), min(line.size(), MESSAGESIZE));

		// Send the message to the server.
		//setting size
		sizeOfMsg = line.size();
		int a = send(sock, (const char*)&sizeOfMsg, sizeof sizeOfMsg, 0);
		if (a == SOCKET_ERROR)
		{
			printf("send error %d", WSAGetLastError());
			die("size send failed");
		}
		int b = send(sock, buffer, min(line.size(), MESSAGESIZE), 0);
		if (b == SOCKET_ERROR)
		{
			printf("send error %d", WSAGetLastError());
			die("msg send failed");
		...
		//int count = recv(sock, buffer, MESSAGESIZE, 0); //original recv (only recieving the data which has arrived at the destination so far, its not waiting for the whole msg) this is fine for local machines since the transfer will be fast af

		//recieving line.size() - 1 because we are not sending the delimiter back hence 1 less character to read
		int count = recv(sock, buffer, line.size(), MSG_WAITALL); //this modification (flag, at the very end) makes the recv wait for the whole msg and then write that to the buffer) when implementing networking use this as this will make sure you recieve the whole data
