...
int main()
{
	...
	do {
		...
		fflush(stdout); //to flush out the contents on the screen (buffer) and get only the useful data the user is gonna type now.
		...
		//memset(buffer, '-', MESSAGESIZE); //not setting dashes, because i want to send clean msg
		...
		if (rand() % 100 < 60) {
			...
			if (sendto(sock, buffer, min(line.size(), MESSAGESIZE), 0,
				(const sockaddr*)&toAddr, sizeof(toAddr)) != min(line.size(), MESSAGESIZE))
			{
				die("sendto failed");
			}
		...
		int count = recvfrom(sock, buffer, min(line.size(), MESSAGESIZE), 0,
			                 (sockaddr *) &fromAddr, &fromAddrSize);
		if (count < 0)
		{
			die("recvfrom failed");
		}
		/*if (count != min(line.size(), MESSAGESIZE))
		{
