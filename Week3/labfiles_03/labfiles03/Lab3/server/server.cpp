...
int main()
{
	...
	while (true)
	{
		...
		int count;
		count = recvfrom(sock, buffer, MESSAGESIZE, 0,
				(sockaddr*)&fromAddr, &fromAddrSize);
		if (count < 0)
		{
			die("recvfrom failed");
		}
			//removed for recieving a clean message 
		...
		if (sendto(sock, buffer, count, 0, (const sockaddr *) &fromAddr, sizeof(fromAddr)) != count)
		{
			die("sendto failed");
		}
	}
